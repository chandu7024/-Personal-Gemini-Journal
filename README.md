...
| **TC-10: Location-Aware Pinning** | Click **"+ Location"** or the location badge on an entry workspace. | Opens the Google Maps Sanctuary Picker. User can detect GPS, choose presets, or click on the interactive map to pin coordinates and place names. Grounded reflection context is included with Gemini reflections and isolated in Firestore. |
| **TC-11: Threat Model Viewer** | Click the **"Threat Model"** button on the navbar or landing page. | Displays the full 5 Threat Zones modal detailing active mitigations for OWASP Top 10, LLM vulnerabilities, and Maps key security. |
